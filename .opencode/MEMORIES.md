...
- e2e: needs devenv up + a leader on :18002 with all HNVR_DISABLE_* gates; `cd tests/e2e && npm install && npx playwright install chromium && npm test`. Run from repo root with the leader already started — a missing `./result/bin/hnvr-leader` fails ALL specs fast. Screenshots: `tests/e2e/scripts/screenshots.mjs` (LIVE_URL=:18001, FRESH_URL=:18002).
...
- Env kill switches for every background role: `HNVR_DISABLE_NODEROLES`, `_HEALTHREPORTER`, `_SNAPSHOTRESPONDER`, `_EVENTWRITER`, `_HEALTHCACHE`, `_COORDINATOR`, `_BROADCASTER`, `_MEDIAMTX`, `_RETENTION`, `_PENDINGPURGE`, `_METRICS`, `_SNAPSHOTWRITER`, `_PTZSTATUSCACHE`, `_PTZAUDIT`, `_ONVIFSYNC` — used for bisect and for the :18002 test leader.
