...
- Current version **0.25.9.0** (single source of truth: `hnvr-web/hnvr-web.cabal` `version:` — bump on every feature/fix; `Hnvr.Web.version` re-exports via `Paths_hnvr_web` which MUST be in both `autogen-modules` and `other-modules` or link fails).
...
- Tests: ~468 Haskell + 35 Playwright e2e + 1 NixOS smoke.
...
- **hnvr-admin** (package `hnvr-admin/`, nix-only like hnvr-web): management front door (M3). IHP is single-app per binary — admin's `AdminWeb.FrontController` is an intentional orphan `FrontController RootApplication` instance; never import Hnvr.Web.FrontController there. Controllers under `Web.Controller.*` again (pitfall #59); Web.Controller.Sessions/Views are home-package shadows of hnvr-web's (View (NewView User) instance too). Cookie `hnvr_admin`. `AdminWeb.Server.runAdmin` replicates IHP.Server.run with Warp.setHost (IHP has no bind-addr option) + wai-app-static for /static. Bootstrap: `hnvr-admin create-user --email --password` (parses args BEFORE IHP — no pitfall-#122 boot) or INITIAL_ADMIN_* env. Grant tables (composite PK) have NO IHP models — raw SQL in AdminWeb.Grants; roles/admin_audit are Generated (Schema.sql). Every mutation → admin_audit row + NOTIFY roles_events. Last-superadmin guards: pure fns in Hnvr.Core.Authz (canDeleteRole/canRemoveSuperadminGrant/canDeleteUser). pg-simple 0.7: `withConnection` is GONE — bracket+connectPostgreSQL. **M4: leader is read-mostly — /Cameras, /Rules, /PtzPresets (incl. ShowCamera) exist ONLY on hnvr-admin; admin connects NATS and republishes assigns on mutation (busRegistry shared).** `runLeaderMigrations` takes PG advisory lock 727272 — concurrent leader+admin cold boots race schema_migrations otherwise (23505). **Sub-path reverse-proxy mount (0.25.8, BOTH apps)**: leader `HNVR_BASE_URL=https://host/hnvr`, admin `HNVR_ADMIN_BASE_URL=https://host/admin` (full URLs, path allowed). `Hnvr.Core.BasePath.splitBaseUrl` (pure, tested) splits origin/prefix; shared WAI glue is `Hnvr.Web.BasePath` (AdminWeb.BasePath re-exports it). The strip middleware removes the prefix segment-wise before static+auth+router (outside-prefix = 404) and stashes it in the request vault; views MUST emit every root-relative URL via `urlFor` (identity at root mount); Config sets `BaseUrl` option + `APPROOT` env (IHP redirectToPath is approot-based). **Leader uses `mountMiddleware` which also serves `<prefix>/static/*` itself** — `IHP.Server.run`'s staticRouteShortcut sits OUTSIDE the CustomMiddleware stack, so a stripped /static/* misses on disk (admin's custom runAdmin wraps the strip outside its own shortcut instead). Leader JS: `<body data-base-path>` + `HNVR.u(path)` in app.js — every root-relative URL in app.js/timeline.js/ptz.js goes through it (incl. `/whep/<slug>`). /healthz, /status, /debug-frame, /whep all move under the prefix on the leader. EXCEPTION: admin's Rules/New `/debug-frame/<uuid>` JS stays unprefixed — it's a LEADER route on the same vhost. nginx: `proxy_pass http://upstream;` with NO URI part (app strips; commented blocks in nginx.example.conf). Playwright CANNOT e2e a prefix mount via BASE_URL: `page.goto('/x')` drops the baseURL path — use absolute URLs (pattern in /tmp/prefix-smoke.mjs run).
...
- Env kill switches for every background role: `HNVR_DISABLE_NODEROLES`, `_HEALTHREPORTER`, `_SNAPSHOTRESPONDER`, `_EVENTWRITER`, `_HEALTHCACHE`, `_COORDINATOR`, `_BROADCASTER`, `_MEDIAMTX`, `_RETENTION`, `_PENDINGPURGE`, `_METRICS`, `_SNAPSHOTWRITER`, `_PTZSTATUSCACHE`, `_PTZAUDIT`, `_ONVIFSYNC`, `_AUTHZ`, `_ANALYSISWATCHDOG` — used for bisect and for the :18002 test leader.
...
- 128. **Audio trun duration collapse (2026-08-31, "archive silent, live fine")**: Hik-OEM audio RTP DTS jumps backward ~112 ms every ~2 s; ffmpeg clamps to prev+1, and the fMP4 muxer then writes audio trun sample durations of ~22 ticks instead of 1024 — media time creeps at ~2% of wall in stretches (sample COUNT stays 45-49/frag, content fully present). The player's A/V-slope probe reads those windows as legacy and strips audio → striked/disabled volume icon. Fix: `asetpts=NB_CONSUMED_SAMPLES/SR/TB` after `aresample=48000` in the recorder chain (`Hnvr.Capture.Ffmpeg`; NOT `N/SR/TB` — asetpts N is frame index = 1-tick steps). Verified vs live relay: all packets duration 1024, zero DTS clamps. Pre-fix recordings keep full audio content with broken durations — recoverable by rewriting trun durations to 1024 + tfdt rebase (player-side repair NOT implemented).
- 129. **No cabal.project.freeze → hackage index drift breaks vendored minio-hs**: http-client-tls ≥0.3.6.4 moved `Default TLSSettings` from data-default-class to data-default; vendored minio-hs now imports `Data.Default` (not `Data.Default.Class`) and deps `data-default`. Symptom: `No instance for 'Data.Default.Class.Default Conn.TLSSettings'` on a fresh solve.
- 130. **Abandoned `ProcessHandle` = guaranteed zombie** (GHC has no SIGCHLD auto-reaper): every ffmpeg lifetime MUST be bracketed with `Hnvr.Capture.Process.reapProcess` (SIGTERM → 3 s grace poll → SIGKILL → wait, all under `uninterruptibleMask_`) — a supervisor cancellation lands mid-`hGetSome` and skips a trailing `waitForProcess`. `process-1.6.26` exports no `killProcess` — SIGKILL goes through `System.Posix.Signals.signalProcess` + `System.Process.Internals.withProcessHandle`. `Hnvr.Web.AudioProbe.collectPackets` already brackets via typed-process `stopProcess`.
- 131. **No blocking S3 calls in the analyzer sink** (2026-09-01 wedge: `hnvr.frames.<id>` silent until process restart): `uploadThumbnail` ran inline in `analysisSink`; minio-hs retries a dead endpoint forever (#108) so the analyzer async wedged mid-event → frames/snapshots/rule events all stopped. Fixed in 0.25.7: `queueThumbnailUpload` forks a bounded (8 in-flight) 60 s-timeout upload and publishes the event inline immediately; `publishFrame` swallows NATS throws (async-rethrow first); `analysisWatchdog` (30 s scan, `HNVR_DISABLE_ANALYSISWATCHDOG=1` gate) restarts a pair whose async died or whose newest analyzed frame is >120 s stale while the worker is Running — restarts go through the camera lock + `csAssignments` re-check, capture worker untouched.
- 132. **IHP IGNORES `setCookieName` in the SessionCookie option** — `initSessionMiddleware` is `withSession store "SESSION" …` (IHP/Server.hs), so both apps read/write `SESSION; Path=/` and share sessions on a shared vhost (leader at / + admin at sub-path = cross-login). Fixed in 0.25.9: `Hnvr.Core.SessionCookie` (pure, tested) + `Hnvr.Web.SessionCookie.sessionCookieMiddleware` translate app-cookie ↔ SESSION at the CustomMiddleware edge (outermost slot — inbound rename own→SESSION + DROP foreign SESSION, outbound rename SESSION→own). Cookie names: leader `hnvr`, admin `hnvr_admin`. Renaming invalidated existing leader sessions once. Cookie values may contain `=` padding — split pairs on the FIRST `=` only.

- 133. mediamtx API can wedge while RTSP keeps serving fine (seen 2026-09-04 on snp-srv, v1.20.0, 4-day uptime): symptom = `MediaMTXConfigSyncer: REST push failed … ResponseTimeout` every 30 s and new camera "no signal" — yml on disk has the path but running mediamtx logs "path '…' is not configured"; curl to :9997 hangs from the host itself. **Root cause: upstream #6077 — permanent api.mutex ABBA deadlock when config changes via file+API in parallel (the syncer does both); journal shows reloads stop entirely once wedged (last was "3× API request + 1× file changed" in the same second). Fixed upstream in v1.20.1 (#6101) — we bumped the design lock to 1.20.1 (2026-09-04).** Emergency fix pre-bump: `systemctl restart mediamtx` — ExecStartPre only copies the stub when the file lacks the syncer marker, so the leader-rendered config (incl. new paths) survives; capture workers reconnect in seconds.

- 135. **Alive-but-blind analysis pair** (snp-srv/nixos 2026-09-21, "no events for 2 days"): pair restarted via ConfigWatcher broadcast while the leader was down; afterwards inference ran, frames published, watchdog happy (ahLatest fresh) — but ZERO rule events until the node restarted. `hnvr.events` carried only Video segment events; DB `events.max(created_at)` froze. Node restart healed it instantly. Diag chain that worked: DB events max → bus subscribe for non-Video swKind → out-of-band ORT inference on a grabbed frame (model+input fine) → node's own SnapshotResponder reply has valid rules → restart. No root cause for the wedge itself (state in `evalTracks`/tracker refs?); if it recurs, unit-test `Hnvr.Cv.Rules.evalTracks` with the live snapshot rules. **Fixed in 0.25.14.0: `hnvr_rule_events_total{camera}` + `hnvr_tracks_active{camera}` metrics, and a blind-pair check in analysisWatchdog (pure `Hnvr.Core.AnalysisLiveness.blindVerdict`)** — tracks recently active but no rule event for `HNVR_BLIND_EVENT_QUIET_SEC` (default 7200) → restart pair; cameras quiet longer than `HNVR_BLIND_TRACK_QUIET_SEC` (default 1800) never restart on missing events. Same kill switch as the watchdog: `HNVR_DISABLE_ANALYSISWATCHDOG=1`.

- 134. **`ReadWritePaths` on a missing dir = status=226/NAMESPACE, restart loop** (snp-srv 2026-09-19, both hnvr-leader + mediamtx wedged): `/run/hnvr` comes from a tmpfiles `d` rule which only runs at boot/switch — a service restart after the dir vanished fails before ExecStartPre. Fix: `RuntimeDirectory="hnvr"` (+`RuntimeDirectoryMode="0750"`) in BOTH units — systemd creates it pre-namespace at every start and reference-counts the shared dir across units. Nix gotcha: `serviceConfig = {…} // optionalAttrs …` breaks attrpath merging with a sibling `serviceConfig.X = …` definition ("already defined") — keep the literal pure, use definition-level `lib.mkIf` outside.
