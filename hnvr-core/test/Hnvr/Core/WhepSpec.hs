...
tests =
  testGroup
    "Hnvr.Core.Whep"
    [ testGroup
        "translatePath (/whep/<slug> → /<slug>/whep)"
      ...
      testGroup
        "translateBack (/<slug>/whep → /whep/<slug>)"
        [ testCase "path-only Location" $
            assertEqual
              "path only"
              "/whep/cam-197/session/abc"
              (translateBack "/cam-197/whep/session/abc"),
          testCase "absolute http URL strips scheme + host" $
            assertEqual
              "http absolute"
              "/whep/cam-197/session/abc"
              (translateBack "http://127.0.0.1:8889/cam-197/whep/session/abc"),
          testCase "absolute https URL strips scheme + host" $
            assertEqual
              "https absolute"
              "/whep/cam-197/session/abc"
              (translateBack "https://leader.example/cam-197/whep/session/abc"),
      ...
      testGroup
        ...
        [ testCase "translateBack . translatePath == id (browser form)" $
          ...
          testCase "translatePath . translateBack == id (mediamtx form)" $
            ...
            -- (/<slug>/whep/session/<id>). Back then forward should
            -- round-trip.
            let m = "/cam-197/whep/session/abc"
