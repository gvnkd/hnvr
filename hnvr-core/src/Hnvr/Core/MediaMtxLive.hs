...
-- The first argument lists public ICE hosts to advertise
-- (@webrtcAdditionalHosts@) — without one, mediamtx offers only its
-- interface addresses (LAN IP) and WHEP live view never connects
-- from outside the LAN.
renderPathsYaml :: [Text] -> Text -> [CameraPath] -> Text
renderPathsYaml webrtcHosts relayBase cams =
  T.unlines $
    [ "# Auto-generated by HNVR MediaMTXConfigSyncer. Do not edit.",
      ...
      <> [ "webrtcAdditionalHosts: [" <> T.intercalate ", " (map (\h -> "'" <> h <> "'") webrtcHosts) <> "]"
         | not (null webrtcHosts)
         ]
      <> [ "webrtcEncryption: no",
           "webrtcAllowOrigins: ['*']",
