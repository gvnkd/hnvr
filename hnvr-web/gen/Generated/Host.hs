...
instance FromRow Generated.ActualTypes.Host where
    fromRow = do
        ...
        healthJson <- field
        createdAt <- field
        let theRecord = Generated.ActualTypes.Host id displayName gpuModel execProviders isLeader lastHealthAt healthJson createdAt def { originalDatabaseRecord = Just (Data.Dynamic.toDyn theRecord) }
        pure theRecord
...
instance Record Generated.ActualTypes.Host where
    {-# INLINE newRecord #-}
    newRecord = Generated.ActualTypes.Host def def def def False def def def  def
...
instance SetField "displayName" (Host') Text where
    {-# INLINE setField #-}
    setField newValue record = record { displayName = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 2 } }
instance SetField "gpuModel" (Host') (Maybe Text) where
    {-# INLINE setField #-}
    setField newValue record = record { gpuModel = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 4 } }
instance SetField "execProviders" (Host') [Text] where
    {-# INLINE setField #-}
    setField newValue record = record { execProviders = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 8 } }
instance SetField "isLeader" (Host') Bool where
    {-# INLINE setField #-}
    setField newValue record = record { isLeader = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 16 } }
instance SetField "lastHealthAt" (Host') (Maybe UTCTime) where
    {-# INLINE setField #-}
    setField newValue record = record { lastHealthAt = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 32 } }
instance SetField "healthJson" (Host') (Maybe Data.Aeson.Value) where
    {-# INLINE setField #-}
    setField newValue record = record { healthJson = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 64 } }
...
instance UpdateField "displayName" (Host') (Host') Text Text where
    {-# INLINE updateField #-}
    updateField newValue record = record { displayName = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 2 } }
instance UpdateField "gpuModel" (Host') (Host') (Maybe Text) (Maybe Text) where
    {-# INLINE updateField #-}
    updateField newValue record = record { gpuModel = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 4 } }
instance UpdateField "execProviders" (Host') (Host') [Text] [Text] where
    {-# INLINE updateField #-}
    updateField newValue record = record { execProviders = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 8 } }
instance UpdateField "isLeader" (Host') (Host') Bool Bool where
    {-# INLINE updateField #-}
    updateField newValue record = record { isLeader = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 16 } }
instance UpdateField "lastHealthAt" (Host') (Host') (Maybe UTCTime) (Maybe UTCTime) where
    {-# INLINE updateField #-}
    updateField newValue record = record { lastHealthAt = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 32 } }
instance UpdateField "healthJson" (Host') (Host') (Maybe Data.Aeson.Value) (Maybe Data.Aeson.Value) where
    {-# INLINE updateField #-}
    updateField newValue record = record { healthJson = newValue, meta = record.meta { touchedFields = record.meta.touchedFields .|. 64 } }
...
instance FieldBit "displayName" (Host') where fieldBit = 2
instance FieldBit "gpuModel" (Host') where fieldBit = 4
instance FieldBit "execProviders" (Host') where fieldBit = 8
instance FieldBit "isLeader" (Host') where fieldBit = 16
instance FieldBit "lastHealthAt" (Host') where fieldBit = 32
instance FieldBit "healthJson" (Host') where fieldBit = 64
instance FieldBit "createdAt" (Host') where fieldBit = 128
