...
module Generated.Statements.FetchAdminAudit (statement) where
...
statement :: Statement.Statement (Id' "admin_audit") (Maybe Generated.ActualTypes.AdminAudit)
statement = Statement.preparable sql encoder decoder

sql :: Text
sql = "SELECT id, actor_id, action, object_kind, object_id, payload, created_at FROM admin_audit WHERE id = $1 LIMIT 1"

encoder :: Encoders.Params (Id' "admin_audit")
encoder = Encoders.param (Encoders.nonNullable Mapping.encoder)

decoder :: Decoders.Result (Maybe Generated.ActualTypes.AdminAudit)
