...
sql touchedFieldsList =
    ...
    in "INSERT INTO hosts (id, display_name, gpu_model, exec_providers, is_leader, last_health_at, health_json, created_at) VALUES "
        <> Text.intercalate ", " valueGroups
        <> " RETURNING id, display_name, gpu_model, exec_providers, is_leader, last_health_at, health_json, created_at"
  where
    columnMeta = [(0, False), (1, False), (2, False), (3, True), (4, True), (5, False), (6, False), (7, True)]
...
singleEncoder touchedFields = mconcat $ catMaybes
    ...
    , Just ((.displayName) >$< Encoders.param (Encoders.nonNullable Encoders.text))
