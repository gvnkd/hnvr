...
rowDecoder = do
    ...
    displayName <- Decoders.column (Decoders.nonNullable Decoders.text)
    ...
    pure (let theRecord = Generated.ActualTypes.Host id displayName gpuModel execProviders isLeader lastHealthAt healthJson createdAt def { originalDatabaseRecord = Just (Data.Dynamic.toDyn theRecord) } in theRecord)
