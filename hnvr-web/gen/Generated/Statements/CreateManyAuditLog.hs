...
module Generated.Statements.CreateManyAuditLog (statement) where
...
sql touchedFieldsList =
    ...
    in "INSERT INTO audit_log (id, user_id, action, target_type, target_id, payload, ts) VALUES "
        <> Text.intercalate ", " valueGroups
        <> " RETURNING id, user_id, action, target_type, target_id, payload, ts"
  where
...
singleEncoder :: Integer -> Encoders.Params Generated.ActualTypes.AuditLog
