...
module Generated.Statements.CreateManyAdminAudit (statement) where
...
sql touchedFieldsList =
    ...
    in "INSERT INTO admin_audit (id, actor_id, action, object_kind, object_id, payload, created_at) VALUES "
        <> Text.intercalate ", " valueGroups
        <> " RETURNING id, actor_id, action, object_kind, object_id, payload, created_at"
  where
    columnMeta = [(0, True), (1, False), (2, False), (3, False), (4, False), (5, False), (6, True)]
...
singleEncoder :: Integer -> Encoders.Params Generated.ActualTypes.AdminAudit
