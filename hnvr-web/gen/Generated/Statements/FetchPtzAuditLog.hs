...
module Generated.Statements.FetchPtzAuditLog (statement) where
...
statement :: Statement.Statement (Id' "ptz_audit_log") (Maybe Generated.ActualTypes.PtzAuditLog)
statement = Statement.preparable sql encoder decoder

sql :: Text
sql = "SELECT id, camera_id, user_id, command, args, source, duration_ms, ok, error, ts FROM ptz_audit_log WHERE id = $1 LIMIT 1"

encoder :: Encoders.Params (Id' "ptz_audit_log")
encoder = Encoders.param (Encoders.nonNullable Mapping.encoder)

decoder :: Decoders.Result (Maybe Generated.ActualTypes.PtzAuditLog)
