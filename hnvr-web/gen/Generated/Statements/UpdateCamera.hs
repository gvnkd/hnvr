...
sql touchedFields returning =
    let setEntries = catMaybes
            ...
            , if testBit touchedFields 8 then Just "password" else Nothing
            , if testBit touchedFields 9 then Just "codec" else Nothing
            , if testBit touchedFields 10 then Just "rtsp_sub_url" else Nothing
            , if testBit touchedFields 11 then Just "rtsp_sub_template" else Nothing
            ...
            , if testBit touchedFields 14 then Just "substream_width" else Nothing
            , if testBit touchedFields 15 then Just "substream_height" else Nothing
...
encoder touchedFields = mconcat (catMaybes
    ...
    , if testBit touchedFields 8 then Just ((.password) >$< Encoders.param (Encoders.nullable Encoders.text)) else Nothing
    ...
    , if testBit touchedFields 11 then Just ((.rtspSubTemplate) >$< Encoders.param (Encoders.nullable Encoders.text)) else Nothing
    ...
    , if testBit touchedFields 14 then Just ((.substreamWidth) >$< Encoders.param (Encoders.nullable (fromIntegral >$< Encoders.int4))) else Nothing
    , if testBit touchedFields 15 then Just ((.substreamHeight) >$< Encoders.param (Encoders.nullable (fromIntegral >$< Encoders.int4))) else Nothing
