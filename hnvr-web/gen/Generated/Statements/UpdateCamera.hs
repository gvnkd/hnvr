...
sql touchedFields returning =
    let setEntries = catMaybes
            ...
            , if testBit touchedFields 39 then Just "ptz_enabled" else Nothing
            , if testBit touchedFields 40 then Just "ptz_profile_token" else Nothing
...
encoder touchedFields = mconcat (catMaybes
    ...
    , if testBit touchedFields 40 then Just ((.ptzProfileToken) >$< Encoders.param (Encoders.nullable Encoders.text)) else Nothing
