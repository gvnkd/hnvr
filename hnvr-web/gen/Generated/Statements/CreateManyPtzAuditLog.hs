...
module Generated.Statements.CreateManyPtzAuditLog (statement) where
...
sql touchedFieldsList =
    ...
    in "INSERT INTO ptz_audit_log (id, camera_id, user_id, command, args, source, duration_ms, ok, error, ts) VALUES "
        <> Text.intercalate ", " valueGroups
        <> " RETURNING id, camera_id, user_id, command, args, source, duration_ms, ok, error, ts"
  where
    columnMeta = [(0, True), (1, False), (2, False), (3, False), (4, False), (5, False), (6, False), (7, True), (8, False), (9, True)]
...
singleEncoder :: Integer -> Encoders.Params Generated.ActualTypes.PtzAuditLog
singleEncoder touchedFields = mconcat $ catMaybes
    ...
    , Just ((.command) >$< Encoders.param (Encoders.nonNullable Encoders.text))
    , Just ((.args) >$< Encoders.param (Encoders.nullable Encoders.jsonb))
    , Just ((.source) >$< Encoders.param (Encoders.nonNullable Mapping.encoder))
    , Just ((.durationMs) >$< Encoders.param (Encoders.nullable (fromIntegral >$< Encoders.int4)))
    , if testBit touchedFields 7 then Just ((.ok) >$< Encoders.param (Encoders.nonNullable Encoders.bool)) else Nothing
    , Just ((.error) >$< Encoders.param (Encoders.nullable Encoders.text))
