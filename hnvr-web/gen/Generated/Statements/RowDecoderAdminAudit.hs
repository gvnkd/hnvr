...
module Generated.Statements.RowDecoderAdminAudit (rowDecoder) where
...
import GHC.Records
rowDecoder :: Decoders.Row Generated.ActualTypes.AdminAudit
rowDecoder = do
    id <- Decoders.column (Decoders.nonNullable Mapping.decoder)
    actorId <- Decoders.column (Decoders.nullable Decoders.uuid)
    action <- Decoders.column (Decoders.nonNullable Decoders.text)
    objectKind <- Decoders.column (Decoders.nonNullable Decoders.text)
    objectId <- Decoders.column (Decoders.nullable Decoders.text)
    ...
    pure (let theRecord = Generated.ActualTypes.AdminAudit id actorId action objectKind objectId payload createdAt def { originalDatabaseRecord = Just (Data.Dynamic.toDyn theRecord) } in theRecord)
