...
module Generated.Statements.CreateManyEventClipEvent (statement) where
...
sql touchedFieldsList =
    ...
    in "INSERT INTO event_clip_events (id, clip_id, event_id, created_at) VALUES "
        <> Text.intercalate ", " valueGroups
        <> " RETURNING id, clip_id, event_id, created_at"
  where
    columnMeta = [(0, True), (1, False), (2, False), (3, True)]
...
singleEncoder :: Integer -> Encoders.Params Generated.ActualTypes.EventClipEvent
singleEncoder touchedFields = mconcat $ catMaybes
    ...
    , Just ((.clipId) >$< Encoders.param (Encoders.nonNullable Encoders.uuid))
    , Just ((.eventId) >$< Encoders.param (Encoders.nonNullable Encoders.uuid))
