...
module Generated.Statements.UpdateCameraSnapshot (statement, discardResultStatement) where
...
discardResultStatement :: Integer -> Statement.Statement Generated.ActualTypes.CameraSnapshot ()
...
sql touchedFields returning =
    let setEntries = catMaybes
            ...
            , if testBit touchedFields 2 then Just "ts" else Nothing
            , if testBit touchedFields 3 then Just "object_key" else Nothing
            , if testBit touchedFields 4 then Just "bytes" else Nothing
            , if testBit touchedFields 5 then Just "created_at" else Nothing
            ]
        ...
        returningClause = if returning then " RETURNING id, camera_id, ts, object_key, bytes, created_at" else ""
    in "UPDATE camera_snapshots SET " <> Text.intercalate ", " setClauses <> " WHERE " <> whereClause pkIdx <> returningClause
...
encoder touchedFields = mconcat (catMaybes
    ...
    , if testBit touchedFields 2 then Just ((.ts) >$< Encoders.param (Encoders.nonNullable Encoders.timestamptz)) else Nothing
    , if testBit touchedFields 3 then Just ((.objectKey) >$< Encoders.param (Encoders.nonNullable Encoders.text)) else Nothing
    , if testBit touchedFields 4 then Just ((.bytes) >$< Encoders.param (Encoders.nonNullable (fromIntegral >$< Encoders.int8))) else Nothing
