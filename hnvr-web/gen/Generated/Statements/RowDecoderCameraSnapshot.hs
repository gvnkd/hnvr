...
module Generated.Statements.RowDecoderCameraSnapshot (rowDecoder) where
...
import GHC.Records
rowDecoder :: Decoders.Row Generated.ActualTypes.CameraSnapshot
rowDecoder = do
    ...
    objectKey <- Decoders.column (Decoders.nonNullable Decoders.text)
    ...
    pure (let theRecord = Generated.ActualTypes.CameraSnapshot id cameraId ts objectKey bytes createdAt def { originalDatabaseRecord = Just (Data.Dynamic.toDyn theRecord) } in theRecord)
