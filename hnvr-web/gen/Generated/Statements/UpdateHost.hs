...
sql touchedFields returning =
    let setEntries = catMaybes
            [ if testBit touchedFields 1 then Just "gpu_model" else Nothing
            ...
            , if testBit touchedFields 4 then Just "last_health_at" else Nothing
            , if testBit touchedFields 5 then Just "health_json" else Nothing
...
encoder touchedFields = mconcat (catMaybes
    [ if testBit touchedFields 1 then Just ((.gpuModel) >$< Encoders.param (Encoders.nullable Encoders.text)) else Nothing
    ...
    , if testBit touchedFields 4 then Just ((.lastHealthAt) >$< Encoders.param (Encoders.nullable Encoders.timestamptz)) else Nothing
    , if testBit touchedFields 5 then Just ((.healthJson) >$< Encoders.param (Encoders.nullable Encoders.jsonb)) else Nothing
