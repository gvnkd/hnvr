...
sql touchedFields returning =
    let setEntries = catMaybes
            ...
            , if testBit touchedFields 9 then Just "clip_retention_hours" else Nothing
...
encoder touchedFields = mconcat (catMaybes
    ...
    , if testBit touchedFields 9 then Just ((.clipRetentionHours) >$< Encoders.param (Encoders.nullable (fromIntegral >$< Encoders.int4))) else Nothing
