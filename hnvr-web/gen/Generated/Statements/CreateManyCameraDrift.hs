...
module Generated.Statements.CreateManyCameraDrift (statement) where
...
sql touchedFieldsList =
    ...
    in "INSERT INTO camera_drift (id, camera_id, config_name, field_name, desired, observed, first_seen_at, last_seen_at) VALUES "
        <> Text.intercalate ", " valueGroups
        <> " RETURNING id, camera_id, config_name, field_name, desired, observed, first_seen_at, last_seen_at"
  where
    columnMeta = [(0, True), (1, False), (2, False), (3, False), (4, False), (5, False), (6, True), (7, True)]
...
singleEncoder :: Integer -> Encoders.Params Generated.ActualTypes.CameraDrift
