...
instance FromRow Generated.ActualTypes.User where
    fromRow = do
        ...
        lastLoginAt <- field
        createdAt <- field
        let theRecord = Generated.ActualTypes.User id email passwordHash isAdmin lockedAt failedLoginAttempts lastLoginAt createdAt def { originalDatabaseRecord = Just (Data.Dynamic.toDyn theRecord) }
        pure theRecord
...
instance Record Generated.ActualTypes.User where
    {-# INLINE newRecord #-}
    newRecord = Generated.ActualTypes.User def def def False def def def def  def
