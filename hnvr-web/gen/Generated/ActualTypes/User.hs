...
data User' = User {id :: (Id' "users"), email :: Text, passwordHash :: Text, lockedAt :: (Maybe UTCTime), failedLoginAttempts :: Int, lastLoginAt :: (Maybe UTCTime), timezone :: (Maybe Text), createdAt :: UTCTime, meta :: MetaBag} deriving (Eq, Show)
...
instance IHP.ModelSupport.Table (User') where
    tableName = "users"
    columnNames = ["id","email","password_hash","locked_at","failed_login_attempts","last_login_at","timezone","created_at"]
