...
-- | Shared PTZ control panel markup (Phase 5). Rendered directly on
-- /ShowLive and inside per-camera @\<template\>@s on the dashboard
-- (app.js clones the template into the fullscreen overlay on open).
-- Behaviour lives in @/static/ptz.js@ (@HNVR.ptz(cameraId, root)@).
--
...
ptzPanel camera presets =
  [hsx|
  <div class="card" id="ptz-panel" data-camera-id={cid}>
    <div class="card-header">
      <span>PTZ</span>
      <span id="ptz-status" class="badge badge-mute">…</span>
    </div>
    <div class="card-body ptz-body">
      <div class="ptz-pad">
