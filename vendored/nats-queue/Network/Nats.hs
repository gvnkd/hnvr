...
defaultConnectionOptions =
  NatsConnectionOptions
    { natsConnUser = "nats",
      natsConnPass = "nats",
      natsConnVerbose = True,
      natsConnPedantic = True,
      natsConnSslRequired = False
    }
     ...
     defaultOptions
       { fieldLabelModifier =
           let insertUnderscore acc chr
                 ...
                 | otherwise = chr : acc
            in map toLower . drop 1 . reverse . foldl insertUnderscore [] . drop 8
       }
     ''NatsConnectionOptions
 )
     ...
     defaultOptions
       { fieldLabelModifier =
           let insertUnderscore acc chr
                 | isUpper chr = chr : '_' : acc
                 | otherwise = chr : acc
            in map toLower . drop 1 . reverse . foldl insertUnderscore [] . drop 7
       }
     ''NatsServerInfo
 )
...
ensureConnection ::
  Nats ->
  -- | If true, wait for the connection to become available
  Bool ->
  -- | Action to do when the connection is available
...
prepareConnection nats nhost =
  timeoutThrow timeoutInterval $
    bracketOnError
...
connectionThread ::
  Bool ->
  Nats ->
  -- | inifinite list of connections to try
  [NatsHost] ->
  IO ()
    ...
    errorHandler e = do
      ...
      where
        tryToConnect connlist@(conn : rest) = do
          res <-
            (prepareConnection nats conn >> return (Just connlist))
